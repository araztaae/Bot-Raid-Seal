...
def get_all_sessions(guild_id: int) -> list[dict]:
    ...
    conn = get_connection()
    rows = conn.execute("""
        SELECT s.id, s.guild_id, s.template_key, s.template_name, s.date_time, s.created_by,
...
async def active_session_autocomplete(
        ...
        """,(str(interaction.user.id), interaction.guild_id)).fetchall()
        conn.close()
...
def update_session(session_id: str, **fields) -> bool:
    """
    Update fields in raid_sessions by session_id.
...
async def check_raid_reminders(bot) -> int:
    ...
    for session in sessions:
        ...
        if 0 <= minutes_until <= 10:
            ...
            # Resolve channel — fall back to fetch if not in cache
            try:
                channel = bot.get_channel(int(session["channel_id"]))
                if channel is None:
                    channel = await bot.fetch_channel(int(session["channel_id"]))
            except (discord.NotFound, discord.Forbidden, discord.HTTPException) as e:
                logger.warning(f"Reminder: cannot fetch channel {session['channel_id']}: {e}")
                channel = None
                ...
                try:
                    if mentions:
                        ...
                        await channel.send(
                            f"🔔 **Raid Reminder**\n\n"
                            f"**{session['template_name']}** will begin in less than 10 minutes.\n"
                            f"No players have joined yet!"
                        )
                    sent = True
                    logger.info(f"Reminder sent for session {session['id']} ({session['template_name']})")
                except discord.HTTPException as e:
                    logger.warning(f"Reminder: failed to send message for session {session['id']}: {e}")
...
async def cleanup_expired_sessions(bot) -> int:
    ...
    for session in cur:
        try:
            if not session["channel_id"]:
                logger.warning(f"Cleanup: session {session['id']} has no channel_id, skipping message delete")
            else:
                # Resolve channel — fall back to fetch if not in cache
                try:
                    channel = bot.get_channel(int(session["channel_id"]))
                    ...
                    logger.warning(f"Cleanup: cannot fetch channel {session['channel_id']}: {e}")
                    channel = None

                if channel:
                    # Delete the original raid message so buttons don't linger
                    if session["message_id"]:
                        try:
                            ...
                            pass
                    await channel.send(
                        f"⏰ **{session['template_name']}** initiated by <@{session['created_by']}> has ended automatically."
                    )
                    logger.info(f"Session {session['id']} expired and cleaned up.")
        except Exception as e:
            logger.warning(f"Failed during session expiry cleanup: {e}")
